...
import { InjectionToken, Inject, Injectable, Optional, Directive, ElementRef, Input, Renderer2, NgModule, SkipSelf, Component, defineInjectable, inject } from '@angular/core';
...
var NgxUiLoaderHttpInterceptor = /** @class */ (function () {
    ...
    function NgxUiLoaderHttpInterceptor(config, ngxUiLoaderService) {
        ...
        if (config) {
            if (config.exclude) {
                this.exclude = config.exclude.map(function (url) { return url.toLowerCase(); });
            }
            if (config.excludeRegexp) {
                this.excludeRegexp = config.excludeRegexp.map(function (regexp) { return new RegExp(regexp, 'i'); });
            }
    ...
    function (url) {
        if (this.exclude) {
            ...
            if (this.exclude.findIndex(function (str) { return url.toLowerCase().startsWith(str); }) !== -1) {
                return true;
            }
        }
        if (this.excludeRegexp) {
            // do not show the loader for api urls which matches regexps in the `excludeRegexp` list
            if (this.excludeRegexp.findIndex(function (regexp) { return regexp.test(url); }) !== -1) {
                return true;
            }
        }
        return false;
